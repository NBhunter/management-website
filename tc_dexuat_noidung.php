...
<div class="container-fluid">
 ...
 echo isset($id) ? $id : '' ?>">
			...
			<div class="on-ltchange" >
					...
					<select name="bophan" id="bophan" class="custom-select" readonly>
					<option value="" ></option>
					<?php 
					$lt = $conn->query("SELECT * FROM department order by name asc");
					while($row=$lt->fetch_array()):
					?>
					<option value="<?php echo $row['id'] ?>" <?php echo isset($bophan) && $bophan == $row['id'] ? "selected " : 'disabled' ?> readonly><?php echo $row['name'] ?></option>
					<?php endwhile; ?>
				</select>	
				</div>
					<div class="form-group">
					<label for="" class="control-label">Số tiền</label>
					<textarea name="kinhphi" cols="30" rows="2" class="form-control" readonly><?php echo isset($kinhphi) ? $kinhphi : '' ?></textarea>
					</div>
				<!-- <div class="form-group">
					<label for="" class="control-label">Ngày bắt đầu</label>
					<input type="date" class="form-control" name="thoigianbatdau" value="<?php echo isset($thoigianbatdau) ? date("Y-m-d",strtotime($thoigianbatdau)) : '' ?>"readonly>
				</div>
				<div class="form-group">
					<label for="" class="control-label">Ngày kết thúc</label>
					<input type="date" class="form-control" name="thoigianketthuc" value="<?php echo isset($thoigianketthuc) ? date("Y-m-d",strtotime($thoigianketthuc)) : '' ?>"readonly>
				</div> -->
				<div class="form-group">
				...
				<div class="form-group">
					<label for="" class="control-label">Lý do từ chối</label>
					<textarea name="lydo_tc" cols="30" rows="2" class="form-control"><?php echo isset($lydo_tc) ? $lydo_tc : '' ?></textarea>
				</div>
				</div>
			</div>
		</form>
...
<script>
	...
	// alert($customer_id);
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_tcdexuat_noidung',
			method:'POST',
			data:$(this).serialize(),
			success:function(resp){
				if(resp == 1){
