...
Class Action {
	...
	function get_available(){
		...
		$awhere =" and id != $id ";
		$days=0;
		$leave = $this->db->query("SELECT * FROM leave_list where leave_type_id = $leave_type_id and employee_id= ".$_SESSION['details']['id']." and date_format(date_from,'%Y') = '".date('Y')."'$awhere and status != 2 "); 
...
// ham luu thong tin phieu chi
	...
	function save_customer(){
		...
		$data=" id= '$id' ";
			...
			$data.=", tenkh = '$tenkh' ";
			$data.=", diachi = '$diachi' ";
			$data.=", qlkhuvuc = '$qlkhuvuc' ";		
 			if(empty($id)){
			   $save = $this->db->query("INSERT INTO customer set $data");
			}else{
				$save = $this->db->query("UPDATE customer set $data where makh = $makh");
			}
			if($save)
				return 1;
	}
	function save_dexuat(){
		extract($_POST);
			// $filename = $_FILES['file']['name'];
			
			$data=" id= '$id' ";
			$data.=", tennguoidexuat = '".$_SESSION['details']['id']." ' ";
			$data.=", bophan = '$bophan' ";
			$data.=", noidung = '$noidung' ";
			$data.=", kinhphi = '$kinhphi' ";
			$data.=", thoigianbatdau = '$thoigianbatdau' ";	
			$data.=", thoigianketthuc = '$thoigianketthuc' ";	
			$data.=", Ngaytaophieu = '$Ngaytaophieu' ";	
			
 			if(empty($id)){
			   $save = $this->db->query("INSERT INTO de_xuat set $data");
			}else{
				$save = $this->db->query("UPDATE de_xuat set $data where id = $id");
			}
			if($save)
				return 1;
	}function save_dexuat_noidung(){
		extract($_POST);
		$data=" id= '$id' ";
	...
	function save_tcdexuat_noidung(){
		extract($_POST);
		$data=" status= '2' ";
		$data .=" ,nguoiduyet= ".$_SESSION['details']['id']." ";
		$data .=", thoigianduyet = '".date('Y-m-d')."' ";
			$data.=", lydo_tc = '$lydo_tc' ";
 			if(empty($id)){
			   $save = $this->db->query("INSERT INTO de_xuat set $data");
			}else{
				$save = $this->db->query("UPDATE de_xuat set $data where id = $id");
	...
	function delete_dexuat(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM de_xuat where id = ".$id);
		if($delete){
				return 1;
			}
	}
	function action_dexuat(){
		extract($_POST);
			$data =" nguoiduyet= ".$_SESSION['details']['id']." ";
			$data .=", thoigianduyet = '".date('Y-m-d')."' ";
			$data .=", status = '$status' ";
			$update = $this->db->query("UPDATE de_xuat set $data where id= $id "); 
			if($update)
				return 1;	   
	}
	function decline_dexuat(){
		extract($_POST);
			$data =" nguoiduyet= ".$_SESSION['details']['id']." ";
			$data .=", thoigianduyet = '".date('Y-m-d')."' ";
			$data .=", status = '$status' ";
			$data .=", lydo_tc = '$decline' ";
			$update = $this->db->query("UPDATE de_xuat set $data where id= $id "); 
			if($update)
				return 1;	   
	}
	function action_chidx(){
		extract($_POST);
			$data =" status_chi = '$status_chi' ";
			$update = $this->db->query("UPDATE de_xuat set $data where id= $id "); 
			if($update)
				return 1;	   
	}
	...
	function action_chi(){
		extract($_POST);
			$data =" duyetchi_id = ".$_SESSION['details']['id']." ";
			$data .=", ngayduyet_chi = '".date('Y-m-d')."' ";
			$data .=", status_chi = '$status_chi' ";
			$update = $this->db->query("UPDATE leave_list set $data where id= $id "); 
			if($update)
				return 1;	   
...
// ham luu thong in khach hang
	function save_khachang(){
		...
		if(empty($id)){
			$chk = $this->db->query("SELECT * FROM khach_hang where makh= '$makh' and qlkv = $qlkv ")->num_rows;
			if($chk > 0){
				return json_encode(array("status" => 2,"msg"=>'Khách hàng đã tồn tại.'));
				exit;
			}
			//echo "INSERT INTO employee_details set ".$data;
		if($save)
	...
	function delete_khachhang(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM khach_hang where manv = ".$manv);
		if($delete)
			return 1;
	}
}