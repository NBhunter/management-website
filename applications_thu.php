...
<div class="container-fluid">
	<div class="card">
					...
					<tbody>
						...
						$qry = $conn->query("SELECT ll.*,concat(e.lastname,', ',e.firstname,' ',e.middlename) as name,e.employee_id as eID FROM leave_list ll inner join employee_details e on e.id = ll.employee_id $where and ll.type BETWEEN 4 AND 6 ");
						...
						<tr>
							...
							<?php if($row['type'] == 2): ?>
								...
								if($emp->num_rows > 0 ){
									...
									?>
									<p>Mã nhà cung cấp: <b><?php  echo ($row['id']) ?></b></p>			
									<?php  elseif($row['type'] == 5): ?>
									<?php
										$emp = $conn->query("SELECT *  FROM `customer` WHERE `makh`= '".$row['customer_id']."'");
								if($emp->num_rows > 0 ){
							...
							<?php if($row['status'] == 0): ?>
								...
								<?php endif; ?>
								<p> Người duyệt:<?php echo $action_by ?> </p>
								<p> Ngày:<?php echo $row['status'] > 0 ? date("d/m/Y",strtotime($row['date_approved'])) : 'N/A' ?> </p>
							</td>
							<!-- <td class="text-center">
							...
							<?php if($row['status_chi'] == 0): ?>
								...
								<?php endif; ?>
								<p> Người thu:<?php echo $action_chi ?> </p>
								<p> Ngày:<?php echo $row['status_chi'] > 0 ? date("d/m/Y",strtotime($row['ngayduyet_chi'])) : 'N/A' ?> </p>
							</td class="text-center">
							<?php if($_SESSION['login_id']==19 || $_SESSION['login_id']==14 ){if($row['type'] == 4|| $row['type'] == 5||$row['type'] == 6): ?>
								<td class="text-center">
							<?php if($row['status'] == 0): ?>
								<button class="btn btn-sm btn-outline-primary approved_leave" id="approved_leave" type="button" data-id="<?php echo $row['id'] ?>" >Đồng ý </button>
								<button class="btn btn-sm btn-outline-danger decline_leave" type="button" data-id="<?php echo $row['id'] ?>">Từ chối</button>
