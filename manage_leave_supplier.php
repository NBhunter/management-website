...
<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<h5 class="card-title">Thêm phiếu chi nhà cung cấp</h5>
								</div>
								<div class="card-body">
									<form id="manage-leave-supplier">
										<div class="row">
 ...
 echo isset($id) ? $id : '' ?>">
			...
			<div class="form-group">
				...
				<label for="" class="control-label">Thông tin nhà cung cấp</label>
					...
					<input type="text" name="supplier" list= "browsers" class="form-control">
					...
					?>
					<option value="<?php echo $row['mancc'] ?>"<?php echo isset($customer_id) && $customer_id == $row['mancc'] ? "selected" : '' ?>><?php echo $row['sup_name'] ?></option>
					<?php endwhile; ?>
					</datalist>
				</div>
...
<!-- <div class="container-fluid">
	<div class="col-lg-12">
		<form action="" id="manage-leave-supplier">
...
<script>
	...
	$('#manage-leave-supplier').submit(function(e){
		e.preventDefault()
		if($('.err-msg').length > 0)
			return false;
		start_load()
		$.ajax({
			url:'ajax.php?action=save_leave_supplier',
			method:'POST',
			data:$(this).serialize(),
			success:function(resp){
				if(resp == 1){
